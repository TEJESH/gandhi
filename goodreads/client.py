...
class GoodreadsClient():
    ...
    def book(self, book_id=None, isbn=None):
        ...
        if book_id:
            ...
            return (GoodreadsBook(resp['book'], self).title)+' '+(GoodreadsBook(resp1['book'], self).author)+'\n'+(GoodreadsBook(resp1['book'], self).link)+'\n'
    ...
    def search_books(self, q, page=1, search_field='all'):
       ...
       # print resp
        ...
        #workid = resp['search']['results']['work']
        print works
        
        
        #for work in works:
         #   print self.book(work['best_book']['id']['#text'])
            # If there's only one work returned, put it in a list.
        if type(works) == collections.OrderedDict:
            works = [works]

        #for work in works:
            #print work['best_book']['id']['#text']             #id of book
            #book = self.book(work['best_book']['id']['#text']).link
            #links = book.link
            #print book  
           # return self.book(work['best_book']['id']['#text'])
            #return self.book(work['best_book']['id']['#text']).link

        #return [self.book(work['best_book']['id']['#text']) for work in works] 
        return [self.book(work['best_book']['id']['#text']) for work in works] 
        
    def search_books_links(self, q, page=1, search_field='all'):
        """Get the most popular books for the given query. This will search all
        books in the title/author/ISBN fields and show matches, sorted by
        popularity on Goodreads.
        :param q: query text
       ...
       # print resp
        ...
        #return [self.book(work['best_book']['id']['#text']) for work in works] 
        return [self.book(work['best_book']['id']['#text']).link for work in works]     
